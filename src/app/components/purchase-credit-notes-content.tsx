...
import { Search, FileText, Download, Printer, CheckCircle, Clock, AlertTriangle, XCircle, Plus, Shield, ZoomIn, ZoomOut, RefreshCw, Code2, X } from "lucide-react";
import { toast } from "sonner";
import { useTheme } from "../contexts/theme-context";
import { DatePicker } from "./date-picker-range";
import { CreatePurchaseCreditNoteModal } from "./create-purchase-credit-note-modal";
...
/* ══════════════════════════════════════════════════════════════════════
   FUNCIÓN: GENERAR XML
══════════════════════════════════════════════════════════════════════ */
function generateCreditNoteXML(note: PurchaseCreditNote): string {
...
const CREDIT_NOTES_INIT: PurchaseCreditNote[] = [
  {
    id: "1",
    noteNumber: "004-001-000001",
    date: "2026-03-05",
    time: "10:20",
    ...
    description: "Devolución de 2 laptops con defectos de fábrica detectados en inspección de calidad",
    subtotal: 1700.00,
    tax: 204.00,
    total: 1904.00,
    status: "authorized",
    authorizationNumber: "0503202601179001691900120040010000000141234567890",
  ...
  },
  {
    ...
    reason: "Descuento comercial por volumen",
    supplier: {
    ...
    description: "Descuento del 10% por compra mayor a $5000 según política comercial vigente",
    subtotal: 500.00,
    tax: 60.00,
    total: 560.00,
    status: "authorized",
  ...
  },
  {
    ...
    reason: "Corrección de precio facturado",
    supplier: {
    ...
    description: "Ajuste de precio según lista actualizada enviada el 01/03/2026",
    subtotal: 150.00,
    tax: 18.00,
    total: 168.00,
    status: "pending",
...
function CreditNoteRIDE({ note, zoom, setZoom, onPrint, activeView, setActiveView }: CreditNoteRIDEProps) {
            ...
            <>
              ...
              <button 
                onClick={() => {
                  ...
                  a.href = url;
                  a.download = `nota-credito-${note.noteNumber}.xml`;
                  a.click();
              ...
              <div className="bg-white mx-auto shadow-2xl text-gray-800" style={{ width: 520, fontFamily: "'Segoe UI', Arial, sans-serif", fontSize: 9.5 }}>
                ...
                <div style={{ border: "1px solid #9ca3af", margin: "4px 7px 0", background: "#dbeafe" }}>
                  <div style={{ padding: "4px 7px", display: "flex", alignItems: "center", gap: 5 }}>
                    <XCircle style={{ width: 14, height: 14, color: "#1e40af", flexShrink: 0 }} />
                    <div>
                      <p style={{ fontWeight: 700, fontSize: 8, color: "#1e40af" }}>Documento Modificado:</p>
                      <p style={{ fontFamily: "monospace", fontSize: 8.5, color: "#1e3a8a", fontWeight: 700 }}>Factura {note.invoiceRef}</p>
                      <p style={{ fontSize: 8, color: "#1e40af", marginTop: 1 }}><b>Motivo:</b> {note.reason}</p>
                    </div>
                  </div>
                </div>
...
══════════════════════════════════════════════════════════════════════ */
export function PurchaseCreditNotesContent() {
  const { theme } = useTheme();
  const isLight = theme === "light";

  const [notes, setNotes] = useState<PurchaseCreditNote[]>(CREDIT_NOTES_INIT);
  const [selected, setSelected] = useState<PurchaseCreditNote | null>(CREDIT_NOTES_INIT[0]);
  ...
  const handleSaveNote = (newNote: PurchaseCreditNote) => {
    ...
    <div className="flex flex-col gap-6 h-full">
      ...
      <div className={`flex gap-0 rounded-xl border flex-1 min-h-0 ${isLight ? "border-gray-200 bg-white" : "border-white/10 bg-white/5"}`}>
        ...
        <div className={`flex flex-col border-r flex-shrink-0 min-w-0 rounded-l-xl ${isLight ? "border-gray-200 bg-gray-50" : "border-white/10 bg-[#1a2936]"}`} style={{ width: "60%" }}>
          ...
          <div className={`px-4 py-3 border-b flex-shrink-0 flex flex-wrap items-center gap-2 ${isLight ? "border-gray-200 bg-white" : "border-white/10 bg-[#0D1B2A]"}`}>
            ...
            >
              ...
              <option value="devolucion" className={opt}>Devolución</option>
              <option value="descuento" className={opt}>Descuento</option>
              <option value="correccion" className={opt}>Corrección</option>
            </select>
          ...
          <div className={`px-4 py-2.5 border-b flex-shrink-0 flex items-center gap-2 ${isLight ? "border-gray-200 bg-gray-50" : "border-white/10 bg-[#0D1B2A]"}`}>
            <button 
            ...
            <button 
              onClick={() => {
                if (!selected) {
                  toast.error("Seleccione una nota de crédito para anular");
                  return;
                }
            ...
            >
              <XCircle className="w-3.5 h-3.5" /> Anular Nota de Crédito
            </button>
          </div>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className={`text-sm ${isLight ? "text-gray-500" : "text-gray-400"}`}>No se encontraron notas de crédito</p>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        <div className={`flex flex-col flex-1 min-w-0 rounded-r-xl ${isLight ? "bg-white" : "bg-[#0d1724]"}`}>
          {selected ? (
            <CreditNoteRIDE note={selected} zoom={zoom} setZoom={setZoom} onPrint={handlePrint} activeView={activeView} setActiveView={setActiveView} />
          ) : (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className={`text-sm font-medium ${isLight ? "text-gray-600" : "text-gray-400"}`}>
                  Selecciona una nota de crédito para ver los detalles
                </p>
              </div>
            </div>
          )}
